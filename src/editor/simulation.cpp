...
anyVector compute_viscosity (anyCell *particle_i, anyCell *particle_j)
{
    anyVector rVec = particle_i->pos - particle_j->pos;
    float r_sq = rVec.dot(rVec);
    float r = sqrt(r_sq);
    ...
    anyVector viscosity = rVec*(particle_i->velocity - particle_j->velocity)*(0.5f * (particle_i->fluid_viscosity + particle_j->fluid_viscosity) / particle_j->density);
    viscosity *= Grad_BicubicSpline(rVec, c::H);
    viscosity /= rVec.dot(rVec) + 0.01f*pow(c::H, 2);
    return viscosity;
}

anyVector compute_pressure (anyCell *particle_i, anyCell *particle_j)
{
    anyVector rVec = particle_i->pos - particle_j->pos;
    float r_sq = rVec.dot(rVec);
    float r = sqrt(r_sq);
    ...
    anyVector pressure = ((particle_j->pressureF / pow(particle_j->density, 2) + particle_i->pressureF / pow(particle_i->density, 2))*GradW_spiky(r, c::H)*rVec);
    return pressure;
}
...
void concentration_exchange(float conc1[sat::dsLast][2], float conc2[sat::dsLast][2], float r1, float r2, float dist2, bool bidirectional = true)
{
    ...
    const float max_exchange_ratio = 1/14.0 / 200; // okolo 14 kul tej samej wielkosci zmiesci sie obok danej kuli;
...
*/
{
    ...
    for (int i = 0; i < box1_no_cells; i++){
        anyCell particle_i = scene::Cells[box1_first_cell+i];
        anyVector pressureF (0.0f);
        anyVector viscosityF(0.0f);
        for (int j = 0; j < box2_no_cells; j++){
            pressureF -= compute_pressure(scene::Cells + box1_first_cell + i, scene::Cells + box1_first_cell + j);
            viscosityF += compute_viscosity(scene::Cells + box1_first_cell + i, scene::Cells + box1_first_cell + j);
        }
        particle_i.pressureF = pressureF;
        particle_i.viscosityF = viscosityF;

        particle_i.viscosityF /= particle_i.density;

        anyVector externalF = anyVector(0.0f, c::gravityAcc * particle_i.mass, 0.0f);

        anyVector totalF = pressureF + viscosityF + externalF;
        ...
        for (int box_y = 0; box_y < SimulationSettings.no_boxes_y; box_y++)
            for (int box_x = 0; box_x < SimulationSettings.no_boxes_x; box_x++, box_id++)
            {
                ...
                if (no_cells)
                {
                    // inner-box forces...
                    for (int i = 0; i < no_cells - 1; i++){
                        anyCell particle_i = scene::Cells[first_cell+i];
                        anyVector pressureF (0.0f);
                        anyVector viscosityF(0.0f);

                        for (int j = i + 1; j < no_cells; j++){
                            pressureF -= compute_pressure(scene::Cells + first_cell + i, scene::Cells + first_cell + j);
                            viscosityF += compute_viscosity(scene::Cells + first_cell + i, scene::Cells + first_cell + j);
                        }
...
*/
{
    ...
    // Calculate densities in all cells..., timer: TimerDensitiesId
    CalculateCellsDensities();
